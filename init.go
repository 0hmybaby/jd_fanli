...
var fanli = core.NewBucket("fanli")

func init() {

	core.Server.GET("/fanli/:sku", func(c *gin.Context) {
		sku := c.Param("sku")
		c.String(200, core.OttoFuncs["fanli"](sku))
	})
	...
	core.OttoFuncs["fanli"] = getFanli //类似于向核心组件注册
}
...
func getFanli(url string) string {
	...
	req := httplib.Get("https://api.jingpinku.com/get_rebate_link/api?" +
		"appid=" + fanli.Get("jingpinku_appid") +
		"&appkey=" + fanli.Get("jingpinku_appkey") +
		"&union_id=" + fanli.Get("jd_union_id") +
	...
	var rslt string = title + short + official + price + final
